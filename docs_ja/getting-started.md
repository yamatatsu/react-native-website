...
このページは、最初のReact Nativeアプリのインストールと構築に役立ちます。 React Nativeが既にインストールされている場合は、[Tutorial](tutorial.md)にスキップしてもいいよ。
...
[Create React Native App](https://github.com/react-community/create-react-native-app) は、新しいReact Nativeアプリケーションの構築を開始する最も簡単な方法です。これを使えば、ネイティブコードを構築するためのツールをインストールまたは設定することなく、プロジェクトを開始することができます。つまり XcodeやAndroid Studioのインストールは必要ありません。 (see [Caveats](getting-started.md#caveats))
...
[Node](https://nodejs.org/en/download/) がインストールされている場合、npmを使って `create-react-native-app` をインストールすることができます：
...
次のコマンドを実行して、新しいReact Nativeプロジェクトを作成します。
...
あなたのiOSまたはAndroidフォンに[Expo]（https://expo.io）クライアントアプリケーションをインストールし、お使いのコンピュータと同じワイヤレスネットワークに接続します。 Expoアプリを使用して、端末からQRコードをスキャンしてプロジェクトを開きます。

### Modifying your app

> Now that you have successfully run the app, let's modify it. Open `App.js` in your text editor of choice and edit some lines. The application should reload automatically once you save your changes.

これでアプリケーションを正常に実行したので、修正しましょう。 選択したテキストエディタで `App.js`を開き、いくつかの行を編集します。 変更を保存すると、アプリケーションは自動的にリロードされます。
...
* Create React Native Appには、[ユーザーガイド](https://github.com/react-community/create-react-native-app/blob/master/react-native-scripts/template/README.md) がありますツール固有の質問がある場合は参照できます。

* 正常に動作しない場合は、[トラブルシューティング](https://github.com/react-community/create-react-native-app/blob/master/react-native-scripts/template/README) を参照してください。

React Nativeについてもっと知りたい人は、[Tutorial](tutorial.md) に進んでください。
...
React Create Native Appを使用すると、開発環境を設定せずに物理デバイス上でReact Nativeアプリを実行するのが簡単になります。 iOS SimulatorまたはAndroid仮想デバイスでアプリケーションを実行する場合は、XcodeをインストールしてAndroid開発環境を設定する方法については、ネイティブコードを使用してプロジェクトを作成する手順を参照してください。

これらを設定したら、Android仮想デバイスで `npm run android`を実行するか、iOSシミュレータで` npm run ios`（macOSのみ）を実行してアプリを起動できます。
...
Reactネイティブアプリを使用してプロジェクトを作成するときにネイティブコードを作成しないため、ReactネイティブAPIやExpoクライアントアプリで使用できるコンポーネント以外のカスタムネイティブモジュールを含めることはできません。

しかし、最終的に独自のネイティブコードを組み込む必要があることがわかっている場合でも、Create React Native Appを使用して、開始することはワンチャンあります。その場合は、[eject](https://github.com/react-community/create-react-native-app/blob/master/react-native-scripts/template/README.md#ejecting-from-create-react-native-app) を使用して、最終的に独自のネイティブ・ビルドを作成します。取り出しを行う場合は、「ネイティブコードでプロジェクトを作成する」の手順がプロジェクトで引き続き作業する必要があります。

React Create Native Appは、Expoクライアントアプリケーションでサポートされている最新のReact Nativeバージョンを使用するようにプロジェクトを設定します。 Expoクライアントアプリケーションは、通常、React Nativeバージョンが安定版としてリリースされてから約1週間後に、特定のReact Nativeバージョンをサポートします。 [このドキュメント](https://github.com/react-community/create-react-native-app/blob/master/VERSIONS.md) で、サポートされているバージョンを確認できます。

React Nativeを既存のプロジェクトに統合する場合は、Create React Native Appをスキップして、ネイティブのビルド環境の設定に直接移動します。 React Nativeのネイティブビルド環境を設定する手順については、上記の「ネイティブコード付きプロジェクトのビルド」を選択してください。
...
プロジェクトにネイティブコードを作成する必要がある場合は、以下の手順に従ってください。 たとえば、React Nativeを既存のアプリケーションに統合する場合、またはCreate React Native Appから「取り除いた場合」には、このセクションが必要です。

説明は、開発オペレーティングシステムによって異なります。また、iOSまたはAndroid用に開発を開始するかどうかも異なります。 iOSとAndroidの両方で開発したい場合は、セットアップが少し異なるため、最初から選択する必要があります。
